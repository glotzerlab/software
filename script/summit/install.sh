...
then
curl -SL https://github.com/pybind/pybind11/archive/v2.7.0.tar.gz | tar -xzC $BUILDDIR && \
    cd pybind11-2.7.0 && \
...
# unlike with Docker builds, use the pip cache on summit to reduce time when rerunning the install script
 ...
 git clone --recursive --branch v3.0.0-beta.7 --depth 1 https://github.com/glotzerlab/hoomd-blue hoomd \
    && cd hoomd \
    && mkdir -p build \
    && cd build \
    && export CFLAGS="-mcpu=power9 -mtune=power9" CXXFLAGS="-mcpu=power9 -mtune=power9" \
