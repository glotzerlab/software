...
then
curl -sSLO https://github.com/oneapi-src/oneTBB/archive/v2021.6.0.tar.gz \
    && tar -xzf v2021.6.0.tar.gz -C . \
    && cd oneTBB-2021.6.0 \
    && cmake -S . -B build -DTBB_TEST=off -DTBB_STRICT=off -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ROOT \
    && cmake --build build -j 8  \
    && cmake --install build \
    && cd .. \
    && rm -rf oneTBB-2021.6.0 \
    && rm v2021.6.0.tar.gz \
    || exit 1
fi
...
then
curl -SL https://github.com/pybind/pybind11/archive/v2.10.0.tar.gz | tar -xzC $BUILDDIR && \
    cd pybind11-2.10.0 && \
...
# unlike with Docker builds, use the pip cache on summit to reduce time when rerunning the install script
 ...
 git clone --recursive --branch v3.5.0 --depth 1 https://github.com/glotzerlab/hoomd-blue hoomd \
    && cd hoomd \
    && mkdir -p build \
    && cd build \
    && export CFLAGS="-mcpu=power9 -mtune=power9" CXXFLAGS="-mcpu=power9 -mtune=power9" \
