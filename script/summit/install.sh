...
then
curl -SL https://github.com/pybind/pybind11/archive/v2.8.0.tar.gz | tar -xzC $BUILDDIR && \
    cd pybind11-2.8.0 && \
...
# unlike with Docker builds, use the pip cache on summit to reduce time when rerunning the install script
 ...
 git clone --recursive --branch v3.0.0-beta.10 --depth 1 https://github.com/glotzerlab/hoomd-blue hoomd \
    && cd hoomd \
    && mkdir -p build \
    && cd build \
    && export CFLAGS="-mcpu=power9 -mtune=power9" CXXFLAGS="-mcpu=power9 -mtune=power9" \
    && cmake ../ -DPYTHON_EXECUTABLE="`which python3`" -DENABLE_GPU=on -DENABLE_MPI=on -DENABLE_TBB=off -DBUILD_JIT=off -DBUILD_TESTING=off -DENABLE_MPI_CUDA=off \
    && make install -j8 \
    && cd ../../ \
    && rm -rf hoomd \
    || exit 1