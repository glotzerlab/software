...
then
curl -sSLO https://github.com/oneapi-src/oneTBB/archive/v2021.12.0.tar.gz \
    && tar -xzf v2021.12.0.tar.gz -C . \
    && cd oneTBB-2021.12.0 \
    && cmake -S . -B build -DTBB_TEST=off -DTBB_STRICT=off -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ROOT \
    && cmake --build build -j 32  \
    && cmake --install build \
    && cd .. \
    && rm -rf oneTBB-2021.12.0 \
...
then
 ...
 git clone --recursive --branch v4.6.0 --depth 1 https://github.com/glotzerlab/hoomd-blue hoomd \
    && cd hoomd \
    && mkdir -p build \
    && cd build \
    && export CFLAGS="-march=native" CXXFLAGS="-march=native" \
